    ...
    private static void Main(string[] args)
    {
        ...
        while (i < 10000)
        {
            for (int index = 0; index < monkeys.Count; index++)
            {
                ...
                {
                    long itemLvl = monkeys.Keys.ElementAt(index).Operation(item);
        ...
        for (int ind = 0; ind < monkeys.Count; ind++)
        {
            if (monkeys.Values.ElementAt(ind) == res)
            {
                ...
                Console.WriteLine(monkeys.Values.Max());
...
class Monkey
{
    List<long> startItems = new();
    public List<long> StartItems
    {
    ...
    public Monkey(long[] items, char operation, int opNum, int divisibleBy, bool help)
    {
    ...
    static void AddItemToMonkey(long item, Monkey m)
    {
